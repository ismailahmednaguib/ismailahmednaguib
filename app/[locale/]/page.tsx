// app/[locale]/page.tsx : الرئيسية — كل كلمة من اللوحة
        ...
        <div className="grid">{tracks.map((x) => (
          <div className="card" key={x.slug}><div className="thumb">{x.icon}</div>
            <div className="pad"><b>{x.title}</b><span className="mut">{x.desc}</span>
              <Link className="btn sm" href={`/${locale}/courses?track=${x.slug}`}>{t("homeTrackBtn", "ادخل المسار")}</Link></div></div>))}
        </div></section>
      <section className="wrap sec"><div className="sec-h"><h2>{t("homeFeaturedTitle", "دورات مميزة")}</h2><Link href={`/${locale}/courses`}>{t("homeAllLink", "الكل ←")}</Link></div>
        {courses.length ? (<div className="grid">{courses.map((c) => (<CourseCard key={c.slug} c={c} locale={locale} />))}</div>)
        : (<p className="mut">{t("emptyCourses", "لا توجد دورات بعد.")}</p>)}</section>
      <section className="wrap sec"><div className="sec-h"><h2>{t("homeBooksTitle", "من المكتبة")}</h2><Link href={`/${locale}/library`}>{t("homeAllLink", "الكل ←")}</Link></div>
        {books.length ? (<div className="grid">{books.map((b) => (<BookCard key={b.slug} b={b} locale={locale} />))}</div>)
        : (<p className="mut">{t("emptyBooks", "لا توجد كتب بعد.")}</p>)}</section>
      <section className="wrap sec"><div className="sec-h"><h2>{t("homeNewsTitle", "آخر الأخبار")}</h2><Link href={`/${locale}/news`}>{t("homeAllLink", "الكل ←")}</Link></div>
...
async function getTranslations({ locale }: { locale: string }) {
