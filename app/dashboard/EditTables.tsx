  ...
  return (
    <>
      ...
      <div style={{ overflowX: "auto" }}><table className="tbl"><thead><tr>
        ...
        {cols.map((c) => (<th key={c}>{c}</th>))}<th>تعديل</th><th>حذف</th>
      </tr></thead><tbody>
        {rows.map((r, i) => {
...
export function CoursesTbl({ rows }: { rows: DashboardRow[] }) {
  return <RowTbl rows={rows} cols={["slug", "title", "track", "teacher", "price", "category"]} table="courses" fields={[
...
export function LessonsTbl({ rows }: { rows: DashboardRow[] }) {
  return <RowTbl rows={rows} cols={["courseSlug", "title", "duration"]} table="lessons" fields={[
...
export function BooksTbl({ rows }: { rows: DashboardRow[] }) {
  return <RowTbl rows={rows} cols={["slug", "title", "author"]} table="books" fields={[
    { name: "slug", label: "slug (ثابت)", readonly: true },
...
export function ScholarsTbl({ rows }: { rows: DashboardRow[] }) {
  return <RowTbl rows={rows} cols={["slug", "name", "title"]} table="scholars" fields={[
    { name: "slug", label: "slug (ثابت)", readonly: true },
...
export function NewsTbl({ rows }: { rows: DashboardRow[] }) {
  return <RowTbl rows={rows} cols={["slug", "title", "date"]} table="news" fields={[
    { name: "slug", label: "slug (ثابت)", readonly: true },
...
export function FatwasTbl({ rows }: { rows: DashboardRow[] }) {
  return <RowTbl rows={rows} cols={["q", "scholar"]} table="fatwas" fields={[
